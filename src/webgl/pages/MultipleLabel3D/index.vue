<script setup lang='ts'>
  ...
  const initThree = () => {
    ...
    const dirLightIns = dirLight({ intensity: 2 }) // 环境光颜色、强度
    scene.add(dirLightIns) // 将环境光添加到场景中
    // 创建相机
    const cameraParams = {
    ...
    renderer.outputColorSpace = THREE.SRGBColorSpace // 设置渲染器输出的颜色空间为sRGB，以保持正确的颜色显示。
    // 创建CSS2DRenderer渲染器，用于渲染HTML元素标签
    cssRenderer = css3Render()
...
<template>
  <div class='three-d-view'>
    <div class="tag1 tag" style="display: none;">标签</div>
