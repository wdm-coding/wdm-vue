<script setup lang='ts'>
  ...
  import { perspective,orthographic } from './camera'
  ...
  const initThree = () => {
    ...
    //光源设置
    const directionalLight = new THREE.DirectionalLight(0xffffff, 2) // 平行光颜色、强度
  ...
  // 设置相机类型
  const cameraType = ref< 'Orthographic' | 'Perspective'>('Perspective')
  const setCamera = (type: 'Orthographic' | 'Perspective') => {
    cameraType.value = type
    if (type === 'Orthographic') {
      camera = orthographic(container.value!.clientWidth, container.value!.clientHeight)
...
<template>
  <div class='three-d-view'>
    <div class="btn_wrap">
      <el-button :type="cameraType === 'Orthographic' ? 'primary' : ''" @click="setCamera('Orthographic')">正交相机</el-button>
      <el-button :type="cameraType === 'Perspective' ? 'primary' : ''" @click="setCamera('Perspective')">透视相机</el-button>
    </div>
