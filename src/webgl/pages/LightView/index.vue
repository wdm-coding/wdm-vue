<script setup lang='ts'>
  ...
  const initThree = () => {
    ...
    pointLight.position.set(80,80,80) // 设置光源位置
    // scene.add(pointLight) // 将光源添加到场景中
    // 点光源可视化位置辅助线
    const pointLightHelper = new THREE.PointLightHelper(pointLight, 10,0x00ff00)
    ...
    directionalLight.position.set(100,0,0) // 设置平行光位置x轴正方向
    directionalLight.position.set(0,100,0) // 设置平行光位置y轴正方向
    directionalLight.position.set(0,0,100) // 设置平行光位置z轴正方向
    directionalLight.position.set(100,100,100) // 设置平行光位置斜45度方向
  ...
  const clock = new THREE.Clock() // 创建时钟对象，用于计算动画帧之间的时间差
