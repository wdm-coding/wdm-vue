<script setup lang='ts'>
  ...
  import { createModel,bloomRender } from './model'
  ...
  const initThree = () => {
    ...
    const { bloomPass,composer: composerRen } = bloomRender(params)
    composer = composerRen
    composerRen.addPass(bloomPass)
    bloomPass.strength = 2.0 // 发光强度
    bloomPass.radius = 0.02 // 发光半径
    bloomPass.threshold = 0.5 // 发光阈值
  }
  // 动画循环函数
