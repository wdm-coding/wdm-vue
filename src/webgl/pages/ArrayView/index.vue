<script setup lang='ts'>
  ...
  const initThree = () => {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      mesh.position.set(i * 200,0,0) // 设置网格位置
      scene.add(mesh) // 将网格添加到场景中
      for (let j = 0; j < 15; j++) {
        const mesh = new THREE.Mesh(geometry, material) // 创建网格
        mesh.position.set(i * 200,0,j * 200) // 设置网格位置
        scene.add(mesh) // 将网格添加到场景中
      }
    }
    // 3. 添加点光源
    ...
    pointLight.position.set(180,180,180) // 设置光源位置
    scene.add(pointLight) // 将光源添加到场景中
    // 添加环境光
    const ambientLight = new THREE.AmbientLight(0xffffff, 2) // 环境光颜色、强度
    ...
    camera = new THREE.PerspectiveCamera(
      ...
      1, // 近剪裁面 相机距离近端截面的距离
      8000// 远剪裁面 相机距离远端截面的距离
    )
    // 设置相机位置
    camera.position.set(2000,2000,2000) // 斜45度方向看立方体
