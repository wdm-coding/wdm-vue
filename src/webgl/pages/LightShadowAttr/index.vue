<script setup lang='ts'>
  ...
  const initThree = () => {
    ...
    directionalLight.position.set(300, 600, -350) // 设置平行光位置斜45度方向
    // 平行光阴影相机就是一个正交相机
    directionalLight.shadow.camera.left = -550 // 设置阴影相机的左边界
    directionalLight.shadow.camera.right = 850 // 设置阴影相机的右边界
    directionalLight.shadow.camera.top = 350 // 设置阴影相机的上边界
    directionalLight.shadow.camera.bottom = -550 // 设置阴影相机的下边界
    directionalLight.shadow.camera.near = 1 // 设置阴影相机的近裁剪面
    directionalLight.shadow.camera.far = 1400 // 设置阴影相机的远裁
    scene.add(directionalLight) // 将平行光添加到场景中
    // 阴影贴图属性mapSize
    directionalLight.shadow.mapSize.set(2048, 2048) // 设置阴影贴图的分辨率
    // 弱化边缘
    directionalLight.shadow.radius = 8 // 设置阴影边缘的模糊程度
    // 可视化平行光阴影摄像机辅助对象
    cameraHelperLine(scene,directionalLight.shadow.camera)
    // 创建相机
    const cameraParams = {
